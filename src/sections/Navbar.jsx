...
function Navbar() {
  ...
  return (
    <div className='fixed inset-x-0 z-20 w-full backdrop-blur-lg bg-primary/40'>
      ...
      {isOpen && (
        <motion.div className='lock overflow-hidden text-center sm:hidden' initial={{opacity: 0, x: -10}} animate={{opacity: 1, x: 0}} style={{maxHeight: '100vh'}} transition={{duration: 1}}>
            <nav className='pb-5'>
                <Navigation />
            </nav>
        </motion.div>)}
